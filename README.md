...
- `bud-uploads` for uploaded artifacts under `/app/uploads`

`BUD_UPLOAD_DIR` (or the unprefixed `UPLOAD_DIR` alias) identifies the artifact
directory. The supplied Compose deployment uses `/app/uploads` and mounts the
`bud-uploads` named volume there, so replacing the application container does
not delete runner evidence.

Each suite execution is represented by a `TestRun` and may upload multiple
