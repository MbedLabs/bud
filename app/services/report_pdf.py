...
class ReportRequest:
    ...
    results: list[tuple[str, str, bool, float]] = field(default_factory=list)
...
def _results_table(
    results: list[tuple[str, str, bool, float]], styles: dict[str, ParagraphStyle]
) -> Table:
    data: list[Sequence[object]] = [["Test", "Outcome", "Duration"]]
    for test_class, test_method, passed, duration in results:
        data.append(
            [
                Paragraph(f"{test_class}.{test_method}", styles["body"]),
                Paragraph(
                    f'<font color="{"#16a34a" if passed else "#dc2626"}">'
                    f'{"Passed" if passed else "Failed"}</font>',
                    styles["body"],
                ),
                f"{duration:.2f}s",
            ]
    ...
    if not results:
        data.append([Paragraph("No individual results recorded.", styles["body"]), "", ""])

    table = Table(data, colWidths=[None, 60, 60], repeatRows=1, hAlign="LEFT")
    table.setStyle(
        TableStyle(
            [
                ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
...
def render_report(request: ReportRequest) -> bytes:
    ...
    story.append(
        Paragraph(
            f"{request.overall.total} tests &middot; "
    ...
    if request.run is not None:
        ...
        story.append(_results_table(request.results, styles))
